...
public class PlayerEquipController : MonoBehaviour
{
    ...
    {        
        int weaponIndex = InventoryManager._inst.GetActiveWeaponIndex(index);       
        if(weaponIndex > 0)
        {            
    ...
    public BowCtrl GetBow()
    {
        BowCtrl ctrl = null;
        if(Weapons.Count > activeIndex)
        {
            if(Weapons[activeIndex].TryGetComponent(out ctrl) == false)
            {
                ctrl = null;
            }
        }
